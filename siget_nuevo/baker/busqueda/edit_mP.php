...
if (!empty($_POST)) {
	...
	if ($operacion == 'edicion'){
	  ...
	  $A=mesaPrevia::traer_datos($id);
		...
		$accion=$_SERVER['HTTP_REFERER'].'?operacion=actualizar&id_mesaP='. $id;
		$btn_txt='Actualizar';
	...
	if ($operacion == 'baja'){
	  ...
	  $A=mesaPrevia::traer_datos($id);
		...
		$accion=$_SERVER['HTTP_REFERER'].'?operacion=borrar&id_mesaP='. $id;
		$btn_txt='Borrar';
		$leyenda='Eliminar';
	}
  ...
  <legend><?php echo $leyenda;?></legend>
	 ...
	 if (isset($operacion)){
			if ($operacion == 'edicion' || $operacion == 'baja' ) {
				...
				<input id="id_mesaP" name="id_mesaP" type="text" class="form-control" disabled value="<?php echo $id; ?>"/>
				<?php
			}
		}	
        ?> 
